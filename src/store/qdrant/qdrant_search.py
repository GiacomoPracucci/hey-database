...
class QdrantSearch(StoreSearch):
    ...
    def _search_points_by_type(
        self, vector: List[float], type_value: DocumentType, limit: int
    ):
        """
        Search for points in Qdrant by document type.

        Performs a vector search against the Qdrant database with a filter
        to return only documents of the specified type.
